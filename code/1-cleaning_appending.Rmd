...
author: "Kim Her & Nicholas Feng Schaefer"
output: html_document
---

# Purpose of This Document
In the first part of the document, we were cleaning up the pre- and daily surveys of each group so that we could merge them. The second part of the document focuses on vertically appending the cleaned datasets into one.
...
ice_pre_path <- file.path("..", "data", "ice_pre.xlsx")
ice_daily_path <- file.path("..", "data", "ice_daily.xlsx")
gw_pre_path <- file.path("..", "data", "gw_pre.xlsx")
gw_daily_path <- file.path("..", "data", "gw_daily.xlsx")

# Loading data.
ice_pre <- read_excel(ice_pre_path)
ice_daily <- read_excel(ice_daily_path)
gw_daily <- read_excel(gw_daily_path)
gw_pre <- read_excel(gw_pre_path)
```
...
# Merge ice_pre and ice_daily data frames on 'Respondent ID' using outer join
...
# Time Work
```{r}
appended_shift_start <- appended_df$shift_start

# Create a new datetime string with a dummy date
datetime_strings <- ifelse(!is.na(appended_shift_start), paste0("1970-01-01 ", substr(appended_shift_start, 1, 12), " ", substr(appended_shift_start, 13, 18)), NA)

# Convert to POSIXct using ymd_hms from lubridate
datetime_parsed <- ymd_hms(datetime_strings, tz = "UTC")

# Adjust for the timezone offset manually
appended_shift_start_parsed <- datetime_parsed + hours(as.numeric(substr(appended_shift_start, 14, 15))) + minutes(as.numeric(substr(appended_shift_start, 17, 18)))
```

```{r}
appended_shift_end <- appended_df$shift_end

# Create a new datetime string with a dummy date
datetime_strings <- ifelse(!is.na(appended_shift_end), paste0("1970-01-01 ", substr(appended_shift_end, 1, 12), " ", substr(appended_shift_end, 13, 18)), NA)

# Convert to POSIXct using ymd_hms from lubridate
datetime_parsed <- ymd_hms(datetime_strings, tz = "UTC")

# Adjust for the timezone offset manually
appended_shift_end_parsed <- datetime_parsed + hours(as.numeric(substr(appended_shift_end, 14, 15))) + minutes(as.numeric(substr(appended_shift_end, 17, 18)))
```

```{r}
time_work <- as.numeric(difftime(appended_shift_end_parsed, appended_shift_start_parsed, units = "hours"))

appended_df$time_work_prop <- time_work / 24

appended_df$time_work_std <- ifelse(appended_df$time_work_prop <= 0, appended_df$time_work_prop + 0.5, appended_df$time_work_prop)

appended_df$time_work <- appended_df$time_work_std * 24
```
...
# Creating a Version Without Outliers
```{r}
appended_df_wo_outliers <- appended_df %>% 
    filter(disposable_income <= mean(appended_df$disposable_income, na.rm = TRUE) + 3*sd(appended_df$disposable_income, na.rm = TRUE))
```

# Exporting Appended Dataset
```{r}
appended_df_path <- file.path("..", "data", "appended_df_outliers.xlsx")
appended_df_wo_outliers_path <- file.path("..", "data", "appended_df_wo_outliers.xlsx")

write.xlsx(appended_df, file = appended_df_path)
write.xlsx(appended_df_wo_outliers, file = appended_df_wo_outliers_path)
```
